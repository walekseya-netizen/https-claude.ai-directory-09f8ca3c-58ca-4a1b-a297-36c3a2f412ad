...
в `/tmp` и очищается при перезапуске — для тестового контура это допустимо.
Для постоянного хранения переведите сервис на план **starter**, верните
`KS_DATABASE_PATH=/data/ks.sqlite3` и раскомментируйте блок `disk` в `render.yaml`.
